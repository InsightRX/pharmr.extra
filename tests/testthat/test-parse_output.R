...
test_that("parse_output extracts the control stream", {
  p <- parse_output(lst_path)
...
test_that("parse_output recovers final THETA estimates (rounded) from the .lst", {
  p <- parse_output(lst_path)
...
test_that("parse_output recovers OMEGA and SIGMA covariance matrices", {
  p <- parse_output(lst_path)
...
test_that("parse_output reads the full-precision OFV", {
  p <- parse_output(lst_path)
  expect_equal(p$ofv, 1248.4264065825578, tolerance = 1e-10)
})

test_that("parse_output accepts contents via `code`", {
  code <- readChar(lst_path, file.info(lst_path)$size)
  p <- parse_output(code = code)
  expect_equal(p$theta, c(1.32, 27.9, 181), tolerance = 1e-6)
})

test_that("parse_output errors on a report without a final-estimate section", {
  code <- "$PROBLEM x\n$THETA 1\n\nNM-TRAN MESSAGES\n AN ERROR WAS FOUND\n"
  expect_error(parse_output(code = code), "FINAL PARAMETER ESTIMATE")
})

test_that("parse_output errors when neither input is supplied", {
  expect_error(parse_output(), "NONMEM report file")
})

test_that("parse_output uses the last FINAL section for multi-step estimation", {
  ...
  expect_equal(parse_output(code = code)$theta, 2.0)
})

test_that("parse_output reassembles matrix rows wrapped onto continuation lines", {
  ...
  om <- parse_output(code = code)$omega
...
test_that("parse_output folds an all-dots continuation line into the matrix row", {
...
test_that("parse_output OFV fallback reads #OBJV, not the #OBJT banner", {
  ...
  expect_equal(parse_output(code = code)$ofv, 5335.951, tolerance = 1e-9)
})

# End-to-end: needs NONMEM/Pharmpy to build the model and apply estimates.
test_that("create_model_from_output applies the recovered estimates", {
  skip_if_nonmem_not_available()
  model <- create_model_from_output(lst_path, verbose = FALSE)
...
test_that("create_model_from_output writes model code when save_as is given", {
  skip_if_nonmem_not_available()
  out <- withr::local_tempfile(fileext = ".mod")
  model <- create_model_from_output(lst_path, save_as = out, verbose = FALSE)
