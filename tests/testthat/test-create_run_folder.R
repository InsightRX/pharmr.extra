...
test_that("truthy non-logical force still overwrites (JSON/CLI round-trips)", {
  for(val in list(1, "TRUE", "true")) {
    tmp_dir <- withr::local_tempdir()
    existing_folder <- file.path(tmp_dir, "existing_run")
    dir.create(existing_folder)
    writeLines("stale", file.path(existing_folder, "run.lst"))

    out <- create_run_folder(id = "existing_run", path = tmp_dir,
                             force = val, verbose = FALSE)
...
test_that("falsy and unparseable force do not overwrite", {
  for(val in list(0, "FALSE", "nonsense", character(0))) {
    tmp_dir <- withr::local_tempdir()
    existing_folder <- file.path(tmp_dir, "existing_run")
    dir.create(existing_folder)
    writeLines("keep me", file.path(existing_folder, "run.lst"))
    ...
    expect_error(
      suppressWarnings(
        create_run_folder(id = "existing_run", path = tmp_dir,
                          force = val, verbose = FALSE)
      ),
      "Run folder.*exists"
    )
